...
document.addEventListener('DOMContentLoaded', function() {
    ...
    updateCartDisplay();
    loadProducts();
    checkCashStatus(); // Verificar estado de caja al cargar
});
...
function toggleCashMenu() {
    ...
    // Agregar estilos del menú si no existen
    if (!document.getElementById('cashMenuStyles')) {
        const styles = document.createElement('style');
        styles.id = 'cashMenuStyles';
        styles.textContent = `
            .cash-dropdown-menu {
                background: white;
...
function createOpenCashModal() {
    ...
    if (!document.getElementById('openCashModalStyles')) {
        const styles = document.createElement('style');
        styles.id = 'openCashModalStyles';
        styles.textContent = `
            .modal-overlay {
                position: fixed;
                top: 0;

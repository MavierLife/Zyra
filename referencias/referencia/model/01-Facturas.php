    ...
    class Facturacion extends Conectar{
        ...
        public function agregar_actualizar_detalle_venta($usuarioactual,$txtuuidventa,$txtcodigoprod,$txtcodigobarra,$txttipoventa,$txtcantidad,$txtconcepto){
            ...
            if ($EstaOfertado > 0) {
                ...
                if (isset($PrecioBase)) {
                    if ($TipoVenta == 1) {
                        ...
                        $PrecioNormal =  round(($PrecioBase / $Unidades), 4);
                        $UnidadesVendidas = ($Cantidad * $Unidades);
                    } else {
                        $PrecioVenta = round($PrecioBase, 4);
                        $PrecioNormal = round($PrecioBase, 4);
            ...
            if ($Encontrado == 1 ){
                ...
                $sql->bindValue(9, $PrecioVenta);
                $sql->bindValue(10, $PrecioVentaSinImpuesto);
                $sql->bindValue(11, $PrecioNormal);
                $sql->bindValue(12, $Descuento);
                $sql->bindValue(13, $NoSujetas);
                $sql->bindValue(14, $Excento);
                $sql->bindValue(15, $Gravado);
                $sql->bindValue(16, $GravadoSinImpuesto);
                $sql->bindValue(17, $IVAItem);
                $sql->bindValue(18, $TotalImporte);
                $sql->bindValue(19, $PrecioCosto);
                $sql->bindValue(20, $TotalCosto);
                $sql->bindValue(21, $PagaImpuesto);
                $sql->bindValue(22, $CodigoTributo);
                $sql->bindValue(23, $Tributo);
                $sql->bindValue(24, $PorcentajeImpuesto);
        ...
        public function consultar_membresia_id($codigomembresia){
            ...
            parent::set_names();
                ...
                $CodigoBase64 = base64_encode($codigoLimpio);
                echo("Escaneado: " . $CodigoBase64);
                $sql="SELECT * FROM tblcatalogodeclientes
                        WHERE
                        CodigoTarjeta64 = ?";
                $sql=$conectar->prepare($sql);
                $sql->bindValue(1, $CodigoBase64);
                $sql->execute();
                return $resultado=$sql->fetchAll();
        ...
        function limpiarTexto($texto) {
            $texto = trim($texto);                // Quitar espacios antes/después
        ...
        public function generar_DTE_Factura($tipotransmision, $uuidventa, $sucursalid, $terminalid) {
                ...
                $ClavePlublica = $negocio["ClavePublica"];
                ...
                if ($estado == "PROCESADO") {
                    ...
                    //Ruta Carpeta
                    $fechaCarpeta = date("Y-m-d", strtotime($fecEmi));
                    //$rutaCarpeta = '../DTEjson/' . $fechaCarpeta;
                    $rutaCarpeta = '../../documentos/contabilidad/DTEjson/' . $fechaCarpeta;

                    // Crear carpeta por fecha si no existe
                    if (!file_exists($rutaCarpeta)) {
                        mkdir($rutaCarpeta, 0777, true); // true para crear carpetas anidadas si no existen
                    }
                    
                    // Creando el archivo JSON en el directorio 
                    $json = json_encode($json_data, JSON_PRETTY_PRINT);
                    $ruta = $rutaCarpeta. '/' . $codigodegeneracion . ".json";
        ...
        public function guardar_total_venta($uuidventa,$txttotalimporte){
            $conectar= parent::conexion();
            parent::set_names();
            $sql="UPDATE tblnotasdeentrega 
                  SET
                ...
                WHERE
                  UUIDVenta = ?";
            $sql=$conectar->prepare($sql);
            $sql->bindValue(1, $txttotalimporte);
            $sql->bindValue(2, $uuidventa);
            if($sql->execute())
			{
                $count = $sql->rowCount();
                if($count == 0){
        ...
        public function update_detalle_venta($txtUUIDDetalleVenta, $txtDescripcion, $txtTipoItem, $txtTipoVenta, $txtPrecioVenta, 
        $txtPrecioVentaSinImpuesto, $txtPrecioNormal, $txtCantidad, $txtUnidadesVendidas, $txtDescuento, $txtTotalImporte, $txtExentas,
        $txtGravadas, $txtGravadasSinImpuesto, $txtIVAItem, $txtPrecioCosto, $txtTotalCosto, $txtUsuarioActual){
        $conectar= parent::conexion();
            parent::set_names();
            ...
            $sql->bindValue(17, $txtUsuarioActual);
            $sql->bindValue(18, $txtUUIDDetalleVenta);
